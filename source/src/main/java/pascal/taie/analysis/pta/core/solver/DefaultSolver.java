...
public class DefaultSolver implements Solver {
                ...
                pts.forEach(array -> {
                    if (array.getObject().isFunctional()) {
                        ...
                        addPFGEdge(new PointerFlowEdge(
                                ...
                                arrayIndex.getType());
                        plugin.onNewArrayStoreEdge(from, arrayIndex);
                    }
                });
            }
        }
            ...
            pts.forEach(recvObj -> {
                if (recvObj.getObject().isFunctional()) {
                    ...
                    if (callee != null) {
                        ...
                        if (!isIgnored(callee)) {
                            addVarPointsTo(calleeContext, callee.getIR().getThis(), recvObj);
                        }
                    } else {
            ...
            if (edge.getKind() != CallKind.OTHER
                ...
                for (int i = 0; i < invokeExp.getArgCount(); ++i) {
                    ...
                    if (propTypes.isAllowed(arg)) {
                        ...
                        addPFGEdge(argVar, paramVar, FlowKind.PARAMETER_PASSING, plugin.getPassEdgeTransfer());
                    }
                }
                ...
                if (lhs != null && propTypes.isAllowed(lhs)) {
                    ...
                    for (Var ret : callee.getIR().getReturnVars()) {
                        if (propTypes.isAllowed(ret)) {
                            ...
                            addPFGEdge(csRet, csLHS, FlowKind.RETURN, plugin.getRetEdgeTransfer());
                        }
                    }
                }
            }
    ...
    @Override
    public PropagateTypes getPropagateTypes() {
        return propTypes;
    }

    @Override
    public Transfer getRetEdgeTransfer() {
        return plugin.getRetEdgeTransfer();
    }

    @Override
    public Transfer getPassEdgeTransfer() {
        return plugin.getPassEdgeTransfer();
    }
