...
public class ArrayModel extends IRModelPlugin {
    ...
    private static final boolean ARRAY_COPY_OPT = false;
    ...
    public List<Stmt> systemArraycopy(Invoke invoke) {
        ...
        Var argSrc = args.get(0);
        Var argDest = args.get(2);
        if (ARRAY_COPY_OPT && ArrayCopyAnalysis.beingAlias(argSrc, argDest)) {
            return List.of();
        }
        ...
        return List.of(
                new Cast(src, new CastExp(argSrc, objArrayType)),
                new Cast(dest, new CastExp(argDest, objArrayType)),
