...
public class Zipper {
    ...
    public static Set<JMethod> run(PointerAnalysisResult pta, String arg) {
        ...
        if (arg.equals("zipper")) {
            isExpress = false;
            pv = 1;
        } else if (arg.equals("zipper-e") || arg.equals("zipper-e-dump")) {
            isExpress = true;
            pv = DEFAULT_PV;
        ...
        Set<JMethod> selectedMethod =  new Zipper(pta, isExpress, pv)
                .selectPrecisionCriticalMethods();
        // dump selected methods
        if (arg.equals("zipper-e-dump")) {
            File outputDir = World.get().getOptions().getOutputDir();
            String filename = "zippere-selected-methods.txt";
            try (PrintStream out =
