...
import pascal.taie.analysis.pta.toolkit.cutshortcut.CutShortcut;
import pascal.taie.analysis.pta.toolkit.cutshortcut.stream.StreamMethods;
...
public class PointerAnalysis extends ProgramAnalysis<PointerAnalysisResult> {
    ...
    public PointerAnalysisResult analyze() {
        ...
        if (advanced != null) {
            ...
            } else if (advanced.equals("stream")) {
                Set<JMethod> collectionMethods = new CollectionMethods(World.get().getClassHierarchy()).get();
                Set<JMethod> streamMethods = new StreamMethods(World.get().getClassHierarchy()).get();
                Set<JMethod> methods = Sets.newSet();
                methods.addAll(collectionMethods);
                methods.addAll(streamMethods);
                selector = ContextSelectorFactory.makeSelectiveSelector(cs, methods);
            } else if (!advanced.startsWith("cut-shortcut")) { // run context-insensitive analysis as pre-analysis
    ...
    private static void setPlugin(Solver solver, AnalysisOptions options) {
        ...
        if (javaVersion >= 8) {
            lambdaAnalysis = new LambdaAnalysis();
            plugin.addPlugin(lambdaAnalysis);
        }
        if (javaVersion >= 9) {
            plugin.addPlugin(new Java9StringConcatHandler());
        }
        if (options.getString("advanced") != null
                && options.getString("advanced").startsWith("cut-shortcut")) {
            plugin.addPlugin(new CutShortcut(options.getString("advanced"),
                    lambdaAnalysis, options.getString("distinguish-string-constants")));
        }
