...
abstract class AbstractPointer implements Pointer {
    ...
    public final Set<Pointer> predecessors = new HybridIndexableSet<>(true);

    private final ArrayList<PointerFlowEdge> outEdges = new ArrayList<>(4);

    public final ArrayList<PointerFlowEdge> inEdges = new ArrayList<>(4);
    ...
    public PointerFlowEdge addEdge(PointerFlowEdge edge) {
        ...
        // for testing purpose
        AbstractPointer target = (AbstractPointer) edge.target();
        if (target.predecessors.add(edge.source())) {
