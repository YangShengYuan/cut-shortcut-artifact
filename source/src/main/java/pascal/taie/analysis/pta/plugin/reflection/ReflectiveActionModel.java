...
import static pascal.taie.analysis.graph.flowgraph.FlowKind.OTHER;
...
public class ReflectiveActionModel extends AnalysisModelPlugin {
        ...
        mtdObjs.forEach(mtdObj -> {
            ...
            if (target != null && !typeMatcher.isUnmatched(invoke, target)) {
                if (target.isStatic()) {
                    ...
                    recvObjs.forEach(recvObj -> addReflectiveCallEdge(
                            context, invoke, recvObj, target, argsVar));
                }
            }
        });
    }
    ...
    private void addReflectiveCallEdge(
        ...
        Var argHub = new Var(callSite.getContainer(), "[ReflectiveCallEdgeArgHub]: "
                + csCallSite + " -> " + csCallee, TopType.Top, -1);
        ReflectiveCallEdge callEdge = new ReflectiveCallEdge(csCallSite, csCallee, args, argHub);
    ...
    public void onNewCallEdge(Edge<CSCallSite, CSMethod> edge) {
        if (edge instanceof ReflectiveCallEdge refEdge) {
            ...
            if (args != null) {
                ...
                // pass arrayIndex to argHub
                CSVar csArgHub = csManager.getCSVar(callerCtx, refEdge.getArgHub());
                solver.getPointsToSetOf(csArgs).forEach(array -> {
                    ArrayIndex elems = csManager.getArrayIndex(array);
                    solver.addPFGEdge(new PointerFlowEdge(OTHER, elems, csArgHub));
                });
                // pass argHub to params
                passReflectiveArgs(refEdge);
            ...
            if (result != null && isConcerned(callee.getReturnType())) {
                ...
                callee.getIR().getReturnVars().forEach(ret -> {
                    ...
                    solver.addPFGEdge(csRet, csResult, RETURN, solver.getRetEdgeTransfer());
                });
            }
        }
    }

    private void passReflectiveArgs(ReflectiveCallEdge edge) {
        ...
        callee.getIR().getParams().forEach(param -> {
            ...
            if (isConcerned(paramType)) {
                ...
                CSVar csArgHub = csManager.getCSVar(callerCtx, edge.getArgHub());
                solver.addPFGEdge(new PointerFlowEdge(
                                PARAMETER_PASSING, csArgHub, csParam), paramType);
            }
        });
    }
    ...
    public void onNewPointsToSet(CSVar csVar, PointsToSet pts) {
        ...
        reflectiveArgs.get(csVar.getVar()).forEach(refEdge -> {
            pts.forEach(array -> {
                ArrayIndex elems = csManager.getArrayIndex(array);
                CSVar csArgHus = csManager.getCSVar(csVar.getContext(), refEdge.getArgHub());
                solver.addPFGEdge(new PointerFlowEdge(OTHER, elems, csArgHus));
            });
        });
    }
