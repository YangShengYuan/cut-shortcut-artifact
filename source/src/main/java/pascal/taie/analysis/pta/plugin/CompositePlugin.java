...
public class CompositePlugin implements Plugin {
    ...
    private final List<Plugin> shouldAddPlugins = new ArrayList<>();

    private final List<Plugin> onNewArrayStoreEdgePlugins = new ArrayList<>();

    private final List<Plugin> retEdgeTransferPlugins = new ArrayList<>();

    private final List<Plugin> passEdgeTransferPlugins = new ArrayList<>();
    ...
    public void addPlugin(Plugin... plugins) {
        for (Plugin plugin : plugins) {
            ...
            addPlugin(plugin, shouldAddPlugins, "shouldAdd", PointerFlowEdge.class);
            addPlugin(plugin, onNewArrayStoreEdgePlugins, "onNewArrayStoreEdge", CSVar.class, ArrayIndex.class);
            addPlugin(plugin, retEdgeTransferPlugins, "getRetEdgeTransfer");
            addPlugin(plugin, passEdgeTransferPlugins, "getPassEdgeTransfer");
        }
    }
    ...
    public void onNewArrayStoreEdge(CSVar from, ArrayIndex arrayIndex) {
        onNewArrayStoreEdgePlugins.forEach(p -> p.onNewArrayStoreEdge(from, arrayIndex));
    }

    @Override
    public boolean shouldAdd(PointerFlowEdge edge) {
        for (Plugin plugin : shouldAddPlugins) {
            if (!plugin.shouldAdd(edge)) {
                return false;
            }
        }
        return true;
    ...
    public Transfer getRetEdgeTransfer() {
        return ((edge, input) -> {
            ...
            for (Plugin plugin : retEdgeTransferPlugins) {
                result = plugin.getRetEdgeTransfer().apply(edge, result);
            }
            return result;
        });
    }
    ...
    public Transfer getPassEdgeTransfer() {
        return ((edge, input) -> {
            PointsToSet result = input;
            for (Plugin plugin : passEdgeTransferPlugins) {
                result = plugin.getPassEdgeTransfer().apply(edge, result);
            }
            return result;
        });
    }
