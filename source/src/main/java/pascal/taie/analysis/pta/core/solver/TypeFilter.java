...
public class TypeFilter implements Transfer {
    ...
    private boolean isAssignable(Type from, Type to) {
        ...
        if (from instanceof SomeType mutable) {
            for (Type t : mutable.potentialTypes()) {
                if (typeSystem.isSubtype(to, t) || typeSystem.isSubtype(t, to)) {
                    return true;
                }
            }
            return false;
        }
        return typeSystem.isSubtype(to, from);
    }
