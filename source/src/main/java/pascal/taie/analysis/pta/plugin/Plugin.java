...
public interface Plugin {
    ...
    /**
     * Invoked when a new Array_Store edge is added in PFG.
     * For precise modeling in certain plugins, e.g, containerAccessHandler
     */
    default void onNewArrayStoreEdge(CSVar from, ArrayIndex arrayIndex) {
    }

    /**
     * Decides whether an edge should be added to the pointer flow graph.
     * NOTE: If a plugin prevents an edge from being added to the PFG,
     * it must be responsible for the points-to information affected
     * by this edge to ensure soundness.
     *
     * @param edge the pointer flow edge
     * @return {@code true} if {@code edge} should be added to the PFG.
     */
    default boolean shouldAdd(PointerFlowEdge edge) {
        return true;
    }

    /**
     * get the edge transfer on return edge
     */
    default Transfer getRetEdgeTransfer() {
        return Identity.get();
    }

    /**
     * get the edge transfer on param passing edge
     */
    default Transfer getPassEdgeTransfer() {
        return Identity.get();
    }
}
