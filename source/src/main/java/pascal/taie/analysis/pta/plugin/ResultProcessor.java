...
public class ResultProcessor implements Plugin {
    ...
    private static final String CI_APP_RESULTS_FILE = "pta-ci-results.txt";
    ...
    private static void logStatistics(PointerAnalysisResult result) {
        ToIntFunction<Pointer> getSize = p -> p.getObjects()
                .stream()
                .filter(o -> o.getObject().isFunctional())
                .collect(Collectors.toSet())
                .size();
        logger.info("---------------- Pointer analysis statistics: ----------------");
        int varInsens = result.getFunctionalVars().size();
        int varSens = result.getFunctionalCSVars().size();
        logger.info(String.format("%-30s%s (insens) / %s (sens)", "#var pointers:",
                format(varInsens), format(varSens)));
        int objInsens = result.getFunctionalObjects().size();
        int objSens = result.getFunctionalCSObjects().size();
        logger.info(String.format("%-30s%s (insens) / %s (sens)", "#objects:",
                format(objInsens), format(objSens)));
        long vptSizeInsens = sum(result.getFunctionalVars(), v -> result.getPointsToSet(v)
                .stream()
                .filter(Obj::isFunctional)
                .collect(Collectors.toSet())
                .size());
        long vptSizeSens = sum(result.getFunctionalCSVars(), getSize);
        ...
        logger.info("-------------- Pointer analysis app statistics: --------------");
        long appVptSizeInsens = sum(
                result.getFunctionalVars()
                        .stream()
                        .filter(var -> var.getMethod().isApplication())
                        .collect(Collectors.toSet()),
                v -> result.getPointsToSet(v)
                .stream()
                .filter(Obj::isFunctional)
                .filter(obj -> {
                    if (obj instanceof MockObj mockObj) {
                        return !mockObj.getDescriptor().equals(LAMBDA_REPLICATED_DESC);
                    } else {
                        return true;
                    }
                })
                .collect(Collectors.toSet())
                .size());
        logger.info(String.format("%-30s%s (insens)", "#var points-to:", format(appVptSizeInsens)));
        logger.info("--------------------------------------------------------------");
    }
    ...
    private static void dumpPointers(
        ...
        pointers.stream()
                ...
                .forEach(p -> {
                    out.println(p + SEP + Streams.toString(p.objects().filter(o -> o.getObject().isFunctional())));
                    out.println(p + SEP + "<<SPECIAL>>-" + Streams.toString(
                            p.objects().filter(o -> ! o.getObject().isFunctional()).map(o -> o.getObject().getAllocation())));
                });
        out.println();
    }
    ...
    /**
     * Dumps points-to sets for all variables in application code (without contexts).
     */
    private static void dumpCiAppPointsToSet(PointerAnalysisResult result) {
        File outFile = new File(World.get().getOptions().getOutputDir(), CI_APP_RESULTS_FILE);
        try (PrintStream out = new PrintStream(new FileOutputStream(outFile))) {
            ...
            result.getFunctionalVars()
                    .stream()
                    .filter(v -> v.getMethod() != null)
                    .filter(v -> v.getMethod().isApplication())
                    .filter(v -> !v.toString().contains("[Lambda-Precise"))
                    .sorted(Comparator.comparing(toString))
                    .forEach(v -> {
                        List<Obj> pts = result.getAllPointsToSet(v)
                                .stream()
                                .filter(obj -> {
                                    if (obj instanceof MockObj mockObj) {
                                        return !mockObj.getDescriptor().equals(LAMBDA_REPLICATED_DESC);
                                ...
                                .toList();
                        if (!pts.isEmpty()) {
                            StringBuilder oneline = new StringBuilder(toString.apply(v) + SEP + "(" + pts.size() + ")objs: ");
                            for (Obj obj : pts) {
                                if (obj.isFunctional()) {
                                    oneline.append(obj).append(" |,| ");
                                }
                            }
                            String str = oneline.toString();
                            if (str.endsWith(" |,| ")) {
