...
public class PointerAnalysisResultImpl extends AbstractResultHolder
    ...
    @Override
    public Collection<CSVar> getFunctionalCSVars() {
        return csManager.getCSVars()
                .stream()
                .filter(csvar ->  csvar.getVar().getType() != TopType.Top)
                .filter(csVar -> propTypes.isAllowed(csVar.getType()))
    ...
    @Override
    public Collection<Var> getFunctionalVars() {
        return csManager.getCSVars()
                .stream()
                .map(CSVar::getVar)
                .filter(var ->  var.getType() != TopType.Top)
                .filter(propTypes::isAllowed)
                .filter(v -> !v.toString().contains("[Lambda-Precise"))
                .collect(Collectors.toSet());
    }
    ...
    @Override
    public Collection<CSObj> getFunctionalCSObjects() {
        return csManager.getObjects()
                .stream()
                .filter(csObj -> csObj.getObject().isFunctional())
                .collect(Collectors.toSet());
    }
    ...
    @Override
    public Collection<Obj> getFunctionalObjects() {
        return csManager.getObjects()
                .stream()
                .map(CSObj::getObject)
    ...
    @Override
    public Set<Obj> getAllPointsToSet(Var var) {
        return varPointsTo.computeIfAbsent(var, v ->
                removeContexts(csManager.getCSVarsOf(var)
                        .stream()
                        .flatMap(Pointer::objects)));
    ...
    private Set<Obj> removeContexts(Stream<CSObj> objects) {
        ...
        objects.map(CSObj::getObject).filter(Obj::isFunctional).forEach(set::add);
